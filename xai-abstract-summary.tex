...
The next step involves initializing unique NNAs for each property transformation.  The NNAs are trained from the data stored from the previous step, using supervised ML techniques.  The result is a set of trained NNAs that produce classifications for the explainable property transforms.
...
Through a manual review of samples in MNIST, we identified explainable properties related to shapes and characteristics of the digits.  The explainable properties we identified and utilized are listed in the Property column of Fig.~\ref{transsample}.
...
The trained NNAs were implemented using the Python scikit-learn\cite{scikitlearn} version 0.23.2 Multi-Layer Perceptrons.  The prop[erty transform NNAs were trained using the transformed data stored in the MNIST IDX files.
