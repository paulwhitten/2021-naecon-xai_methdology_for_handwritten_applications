...
While we approach XAI for a specific classification problem in the MNIST handwritten digit database, we feel the methodology translates to other problems of explainable classification among a finite set of classes.
...
The explainable architecture used in this work is depicted in Fig.~\ref{voting}.  The architecture consists of Properties, a Voter Model Engine, and an Explainable Artificial Intelligence block.  $I_i$ represents the $i$th input to the architecture.

Explainable properties are defined as descriptive qualities of a sample input that mean something to a user in the problem domain, especially to justify a classification decision.   Explainable properties $1$ through $n$ are outlined in the blue rectangle in Fig.~\ref{voting}.  Each explainable property is identified with a $P_x$ square representing logic for classifications based on the particular property.   $Q_i$ represents the $i$th property's classification output.
...
The first voting scheme we present is probabilistic.  In this scheme, we use a knowledgebase to identify each explainable property transformation's effectiveness to correctly predict a class.   The effectiveness for an explainable property transformation, $i$, to select a particular class, $x$,  is given by $E_{i,x}$ which is equal to the ratio of the number of correct classifications of explainable property transformation $i$ of the class $x$ to the total number of elements of class $x$.   We take the sum of effectiveness metrics, for the properties that select a class, $x$, to obtain a weight, $W_x$, for that class.  The confidence for the class $x$ is given by $C_x=\frac{W_x}{\sum\limits_jW_j}$.  The denominator is the sum of all weights for classes that were selected.  If multiple classes are selected by the properties, the class with the highest confidence is will be chosen by the voter.
...
After training, we again process the training set and populate the XAI knowledgebase.  The knowledgebase stores information on properties, the original training label, and each property's classification results from the property NNA.  Metrics on the effectiveness of each explainable property for classes are also stored in the knowledgebase.  The knowledgebase is used in constructing and providing feedback in the voting strategy as well as for composing the classification rationale in the XAI block.
...
Finally, when test data is presented to the XAI architecture, we evaluate the results and determine if they are sufficient or if more explainable properties or transforms are needed.

\section{Examples}

In this work, we utilized the well-studied  MNIST handwritten digit database to develop and test the methodology.   Further details of experiments with MNIST and examples of classification and explainability follow.

Fig.~\ref{transsample} depicts the explainable properties and corresponding transformations used for the MNIST dataset in this work.  The figure also shows example MNIST digits, $I_i$, and their resulting transforms, $T_i$.
...
The Stroke property is meant to represent the minimal path of the writing implement to trace the digit.  The morphological skeleton,  transformation $T_1$, is a one pixel connected representation preserving the rough endpoints of the digit and providing a representation of the stroke.  We utilized the Lee\cite{Lee1994} algorithm for obtaining skeletons of the MNIST handwritten digits.

$P_2, P_4,$ and $P_5$ are the circle property with corresponding transforms $T_2$ the Hough Circle, $T_4$ the Hough Ellipse and $T_5$ accounting for multiple non-overlapping circles and ellipses in the transform.

$P_3$ is the crossings property, representing the intersection of lines in a digit.  The corresponding transform, $T_3$, for finding crossings involved taking the skeleton and then finding activated pixels with more than two neighbors.   The endpoints property, $P_6$ involved a transform, $T_6$, where the skeleton was taken and then activated pixels with only one neighbor were identified.  Note that the endpoints and crossings shown in Fig.~\ref{transsample} have a single pixel, representing each endpoint or crossing, activated and neighboring pixels partially activated to aid in ML classification.

Property $P_7$ and $P_9$ for enclosed regions involved transform $T_7$ which involved a flood fill and $T_9$ the flood fill of the skeleton.  The line property, $P_8$, involves transform $T_8$ which takes non-overlapping Hough Lines of a digit.
...
We next present three interesting examples of MNIST digits along with the property classifications, property effectiveness, and explainability.  We step through the procedures of the probabilistic voting scheme to present the classification results for the example as well as the explainability rationale and the confidence from XAI.
