...
The first voting scheme we present is probabilistic.  In this scheme, we use the knowledgebase to identify each explainable property transformation's effectiveness to correctly predict a digit.   The effectiveness for an explainable property transformation, $i$, to select a particular digit, $d$,  is given by:
\begin{equation}\label{effectiveness}
E_{i,d}  = \frac{|A|}{|B|}
\end{equation}
where $A$ is the set of correct classifications of explainable property transformation $i$ of the digit $d$ and $B$ is the set of elements of digit $d$.  The weight of the effectiveness for each digit, $d$, is:
\begin{equation}\label{weight}
W_d=\sum_i E_{i, d}
\end{equation}
The confidence for the digit $d$ is calculated by the VME and is given by:
\begin{equation}\label{conf}
C_x=\frac{W_d}{\sum\limits_jW_j}
\end{equation}
...
\hline
 $P_i$ & Property & Vote & $E_{i,3}$ & $E_{i,5}$ & $E_{i,6}$ & $E_{i,9}$ & $X_3$ & $X_5$ & $X_6$ & $X_9$ \\
\hline \cline{0-10}
...
\begin{table}[htbp]
\caption{Explainability for Example 2}
\centering
\begin{tabular}{| c | c | c |}
\hline
 $X_d$ & Confidence & Explainable Description \\
\hline \cline{1-3}
$X_3$ & 8\% & circle property \\ 
\hline
$X_5$ & 1\% & crossing property \\
\hline
$X_6$ & 24\% & endpoint property \\
\hline
$X_9$ & 67\% & stroke and enclosed region properties \\
\hline
\end{tabular}
